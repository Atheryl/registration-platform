...
async function register() {
	while (true) {
		...
		var postData = JSON.stringify({
			...
			'age': ageRange[faker.random.number({'min': 0, 'max': 5})],
			'platform': platform[faker.random.number({'min': 0, 'max': 5})],
		...
		await new Promise(done => setTimeout(() => done(), faker.random.number({'min': 0, 'max': 5}) * 1000));  
	}
}

var items = [232, 203, 107, 74, 80, 231];
var item = items[Math.floor(Math.random() * items.length)];
console.log(item);
console.log(countries[item].n);
//register();
